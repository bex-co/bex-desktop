...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "Bex"),
        "preview" => ("app-icon-preview.ico", "Bex Preview"),
        "nightly" => ("app-icon-nightly.ico", "Bex Nightly"),
        _ => ("app-icon-dev.ico", "Bex Dev"),
    };
