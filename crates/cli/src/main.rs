...
struct Args {
    ...
    /// The paths to open in Bex (space-separated).
    ///
    ...
    /// Bex will attempt to open the paths directly.
    ///
    ...
    wsl: Option<String>,
    /// Not supported in Bex CLI, only supported on the app binary
    ...
    diff: Vec<String>,
    /// Generate shell completions for Bex
    #[arg(long, value_names = ["SHELL"])]
    completions: Option<Shell>,
    /// Uninstall Bex from user system
    #[cfg(all(
...
fn run() -> Result<()> {
    ...
    if args.system_specs {
        let path = app.path();
        let msg = [
            "The `--system-specs` argument is not supported in the Bex CLI, only on the app binary.",
...
mod mac_os {
    ...
    impl InstalledApp for Bundle {
        fn zed_version_string(&self) -> String {
            format!("Bex {} – {}", self.version(), self.path().display(),)
        }
