...
#!/usr/bin/env bash
set -euo pipefail
# Rust passes -nodefaultlibs, so --rtlib alone does not add the archive.
runtime=$(/usr/lib/llvm-21/bin/clang --rtlib=compiler-rt --print-libgcc-file-name)
test -f "$runtime"
exec /usr/lib/llvm-21/bin/clang "$@" "$runtime"
LINKER
...
cat > "$RUNNER_TEMP/bex-sme-check.rs" <<'RUST'
unsafe extern "C" { fn __arm_tpidr2_save(); }
fn main() { unsafe { __arm_tpidr2_save(); } }
RUST
rustc --edition=2024 -C "linker=$RUNNER_TEMP/bex-linux-linker" -C link-arg=-fuse-ld=lld "$RUNNER_TEMP/bex-sme-check.rs" -o "$RUNNER_TEMP/bex-sme-check"
